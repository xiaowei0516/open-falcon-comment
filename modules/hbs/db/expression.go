...
//mysql> select id, expression, func, op, right_value, max_step, priority, note, action_id from expression where action_id>0 and pause=0;
//+----+---------------------------------+---------+----+-------------+----------+----------+--------------------+-----------+
//| id | expression                      | func    | op | right_value | max_step | priority | note               | action_id |
//+----+---------------------------------+---------+----+-------------+----------+----------+--------------------+-----------+
//|  1 | each(metric=qps project=falcon) | all(#3) | == | 3           |        3 |        0 |                    |         2 |
//|  2 | each(metric=qps svc=project)    | all(#3) | <= | 100         |      100 |        5 | project have a bug |         3 |
//+----+---------------------------------+---------+----+-------------+----------+----------+--------------------+-----------+
