...
func (this *SafeLinkedList) HistoryData(limit int) ([]*model.HistoryData, bool) {
	...
	//vs  [ {timestamp:value}, {timestamp:value}]
	return vs, isEnough
}
...
// @return needJudge
//   如果是false不需要做judge，因为新上来的数据不合法
//   如果数据合法，则把数据插入LinkedList，如果tLinkedLis过长，超过配置文件的最大值，则
//   从链表后面删除，链表中始终保持最新的数据
